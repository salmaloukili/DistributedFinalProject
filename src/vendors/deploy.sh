...
az appservice plan create --name ticketFusion --resource-group $RESOURCE_GROUP --sku F1  --location westeurope --is-linux
...
VENDOR="ticketFusionDist"

for i in {0..0}; do
    az webapp create --resource-group $RESOURCE_GROUP --plan ticketFusion --name $VENDOR-$i --container-image-name distimgs.azurecr.io/flask-api:latest
